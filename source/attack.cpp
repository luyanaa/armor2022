...
Attack::Attack(Communicator &communicator, PID &pid, ImageShowClient &isClient) : m_communicator(communicator),
                                                                                   m_is(isClient),
 ...
 */
float Attack::distance(const sort::Track &track, const Target &target) {
 ...
 * 设置是否开启预测
 */
void Attack::enablePredict(bool enable) {
 ...
 * 图像扩展ROI
 */
void Attack::getBoundingRect(Target &tar, cv::Rect &rect, cv::Size &size, bool extendFlag) {
 ...
 * @return true
 */
bool Attack::run(cv::Mat &src, int64_t timeStamp, float gYaw, float gPitch) {
